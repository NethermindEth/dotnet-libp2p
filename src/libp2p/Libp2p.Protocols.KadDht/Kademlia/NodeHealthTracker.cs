        ...
        if (_isRefreshing.TryAdd(nodeHash, true))
        {
            Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    // Success: move the node to the front of its bucket (refresh LRU)
                    OnIncomingMessageFrom(toRefresh);
                }
                catch (OperationCanceledException)
                {
                    // Timeout/unreachable: immediately evict, replacement list will promote the pending candidate
                    routingTable.Remove(nodeHash);
                }
                catch (Exception e)
                {
                    // Failure: evict and log; replacement list will promote the pending candidate
                    routingTable.Remove(nodeHash);
                    if (_logger.IsEnabled(LogLevel.Debug)) _logger.LogDebug($"Error while refreshing node {toRefresh}, {e}");
                }

                // Clear refresh flag; do not remove on success path
    ...
    public void OnIncomingMessageFrom(TNode node)
    {
        ...
        if (addResult == BucketAddResult.Full && toRefresh != null)
        {
            if (SameAsSelf(toRefresh))
            {
                ...
                routingTable.TryAddOrRefresh(_currentNodeIdAsHash, config.CurrentNodeId, out TNode? _);
            }
            else
            {
