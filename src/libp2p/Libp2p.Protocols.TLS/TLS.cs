...
public class TlsProtocol : IProtocol
{
    ...
    public async Task DialAsync(IChannel downChannel, IChannelFactory? channelFactory, IPeerContext context)
    {
        ...
        try
        {
            //await sslStream.AuthenticateAsClientAsync("127.0.0.1", null, SslProtocols.Tls13, true);
        ...
        //         if (upChannel.IsClosed)
        // {
        //     logger?.LogError("upChannel is closed. Unable to write.");
        // }
        // await upChannel.WriteAsync(new ReadOnlySequence<byte>(System.Text.Encoding.UTF8.GetBytes("test message")));
        //  _logger?.LogDebug("its done");
