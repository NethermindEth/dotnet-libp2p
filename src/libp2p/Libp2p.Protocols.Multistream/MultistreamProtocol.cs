...
public class MultistreamProtocol2 : IConnectionProtocol
{
    private readonly ILogger? _logger;
    private const string ProtocolNotSupported = "na";
    public string Id => "/multistream/1.0.0";

    public MultistreamProtocol2(ILoggerFactory? loggerFactory = null)
    {
        _logger = loggerFactory?.CreateLogger<MultistreamProtocol2>();
    }
    ...
    public async Task DialAsync(IChannel channel, IConnectionContext context)
    {
        ...
        {
            _logger?.LogTrace(item.ToString());
        }
        _logger?.LogTrace($"Hello started");
